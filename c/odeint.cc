...
{
	if (this->verbose) printf("x1,x2,nstep = %lg, %lg, %d\n", x1, x2, nstep);
...
//	this->evolve=gsl_odeiv2_evolve_alloc(this->nvar);
	...
	for (int j=1; j<=nsteps; j++) {
		...
		else xnext = x1 + (x2-x1)*j/(1.0*nsteps);
		
		if (this->verbose) printf("%lg %lg %lg %lg -- ", x, x1, x2, xnext);

		status = gsl_odeiv2_driver_apply (this->driver, &x, xnext, this->ynext);
		
		if (this->verbose) printf("%lg %lg %lg %lg %d\n", x1, x2, xnext, this->ynext[0], status);
