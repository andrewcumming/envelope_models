...
*                                                       Version 06.07.12
*                         slightly modified 13.09.12, 08.02.13, 22.03.13
...
*        if (FC1=min in solid) then solid regime, output LIQSOL=5
     ...
     *   CMN=0.6, ! effective neutron mass m_n* / m_n
     *   GAMIMELT=175., ! OCP value of Gamma_i for melting
     *   GAML=1.d3, ! the bigest allowed GAMI in liquid
     *   GAMS=1.d0, ! the smallest allowed GAMI in solid
     *   BOHR=137.036, ! inverse fine-str.const.(Bohr radius in rel.un.)
     *   RSIMELT=140.) ! ion density parameter of quantum melting
      parameter (PI2=PI**2,BOHR2=BOHR**2,BOHR3=BOHR2*BOHR)
      DENS=RHO/11.20587*Zion/CMI ! number density of electrons [au]
      B=GAMAG/BOHR2 ! magnetic field in rel.un.
      TEMR=TEMP/BOHR2 ! T [rel.un.]
      Z13=Zion**.33333333d0
      DENR=DENS/BOHR3 ! number density of electrons [rel.un.]
      DENRI=DENR/Zion ! number density of ions (= of cells)
* Plasma parameters:
      RS=(.75d0/PI/DENS)**.33333333d0 ! r_s - electron density parameter
      RSI=RS*CMI*Zion**2*Z13*AUM ! R_S - ion density parameter
      GAME=1.d0/RS/TEMP ! electron Coulomb parameter Gamma_e
      GAMI=Zion**2*GAME/Z13 ! effective Gamma_i - ion Coulomb parameter
      if (LIQSOL.lt.0.or.LIQSOL.gt.5) stop'EOSMAG: invalid LIQSOL'
...
* is as crude as the linear mixing model for Fharm employed below.
      if ((LIQSOL.eq.0.or.LIQSOL.eq.2).and.TPT2.gt.18.)
     *   stop'EOSMAG: too strong quantum effects in liquid'
...
* (1) ideal electron gas (including relativity, degeneracy, and m.f.):
     ...
     *  DENR1,CHI,FEid,PEid,UEid,SEid,CVE,CHITE,CHIRE,DDENR)
      if (abs(DENR/DENR1-1.).gt.EPS) print*,'EOSMAG Warning: DENS1/DENS'
...
* (2) ideal ion gas (including relativity, degeneracy, and m.f.):
     ...
     *     FIid,PIid,UIid,SIid,CVI,CHITI,CHIRI)
      ...
      if (LIQSOL.eq.1.or.LIQSOL.eq.3.or.LIQSOL.eq.4.or.LIQSOL.eq.5) then
         ...
         FIid=1.5d0*dlog(TPT2/GAMI)-1.323515d0+Fspin
...
* (3) Coulomb+xc nonideal contributions:
     ...
     *     FC2,UC2,PC2,SC2,CV2,PDT2,PDR2)
        if (FC1L.lt.FC1) then ! switch to the liquid; recall it.
          if (GAMI.gt.GAML) stop'EOSMAG: too high GAMI in liquid'
           LIQSOL=4
           FIid=FIidL
           PIid=PIidL
           UIid=UIidL
          ...
          if (GAMI.lt.GAMS) stop'EOSMAG: too low GAMI in solid'
           LIQSOL=5
        endif
      endif
...
*   =========  CHEMICAL POTENTIAL and DENSITY DERIVATIVE  ==========   *
     ...
     *  DENR1,CHI,FEid,PEid,UEid,SEid,CVE,CHITE,CHIRE,DDENR)
      ...
      save
*                                                       Version 23.03.13
* Stems from CHEMPOT v.03.05.07 via CHEMAG8 v.26.05.12 in eosmag12.f
...
*  B - magnetic field (=h\omega_B/mc^2)
      parameter (BOHR=137.036,PI=3.141592653d0) ! universal const.
      parameter (PI2=PI**2,BOHR2=BOHR**2,BOHR3=BOHR2*BOHR)
      parameter (NLMAX1=50,PRECLN=9.,EPS=1.D-4,NI=50) ! accuracy param.
      BT=B/TEMR
      if (BT.lt.10.) then ! reduce max.number of Landau levels
         NLMAX=NLMAX1
      else
      ...
      if (B.gt.0.) then
         PFB=PF0**3/(1.5*B)
         PF=dmin1(PF0,PFB) ! magnetic Fermi momentum
        if (PF.gt.1.d-7) then
           TF=dsqrt(1.d0+PF**2)-1.d0 ! Nonmagn.Fermi T
        else
           TF=0.5d0*PF**2
        endif
         DE=TF+PRECLN*TEMR ! (1+DE)=Upper E bound to int.
         CNL0=DE*(1.d0+0.5d0*DE)/B ! Max contributing Landau number
      else
         CNL0=NLMAX+1
      endif
* Preliminary nonmagnetic calculation
C	print*,'CHEMFIT7',DENR,TEMR,CHI,CMU1
...
*   ----------  Calculation of chemical potential CMU   ------------   *'
    ...
    5 continue
   55 continue
      if (dabs(DENR1-DENR).gt.1.d1*DENR*EPS) ! improvement of 02.09.11
     *  print*,'Warning CHEMAG8: INCORRECT density:',DENR1,'  i.o.',DENR
    6 continue
* Interpolation in order to exclude jump at nonmagnetic/magnetic switch
      Y=(1.d0-(CNL0/CNLMAX)**3)**3 ! the weight corrected on 23.03.13
      X=1.d0-Y
...
*         DDENR=(d n_e / d \mu)_T
     ...
     *     DlnDT,DlnDHH,DlnDTT,DlnDHT)
      ...
      if (CHI.lt.-40.d0) then ! nondegenerate
        ...
        if (TEMR.gt.EPSR) ! this case is not suitable for this code
     *    stop'ELECTMAG: nondegenerate, magnetic, relativistic case'
...
C      CHITE1=TEMR/PR*(dPdT-dPdH*G) ! d ln P / d ln T |_V
     ...
     *     DlnDT,DlnDHH,DlnDTT,DlnDHT)
         print*,'ELECTMAG: Warning: CHITE switched to non-magnetic'
      endif
...
* NB: zero-point term is not included.
      ...
      if (UIid.lt.FIid) stop'MAGNION: SIid < 0 (degenerate ions)'
      PIid=1.d0
      CHITI=1.d0
      CHIRI=1.d0
      call SPINION(GYRO,ZETI,MULTI,Fspin,Uspin,CVspin)
...
*   mixture, S_{mix}/(N_i k) HAS TO BE ADDED EXTERNALLY.
      ...
      if (LIQSOL.ne.1.and.LIQSOL.ne.0) stop'EOSFIM12: invalid LIQSOL'
      if (CMI.le..1) stop'EOSFIM12: too small CMI'
      if (Zion.le..1) stop'EOSFIM12: too small Zion'
      if (RS.le..0) stop'EOSFIM12: invalid RS'
      if (GAMI.le..0) stop'EOSFIM12: invalid GAMI'
      GAME=GAMI/Zion**1.666667
      call EXCORM(RS,GAME,GAMAG,
     *  FXC,UXC,PXC,CVXC,SXC,PDTXC,PDRXC) ! "ee"="xc"
* Calculate "ii" part:
...
*                                                       Version 08.02.13
* Stems from v.28.04.12. Typo is corrected!
